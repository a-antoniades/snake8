...
# create dataset
df__ = create_dataset("/Users/antonis/Desktop/PycharmProjects/deep_snake/snake_data/df_snake_test")
df = pd.DataFrame(df__)
df_1 = df.loc[df[5] == 1]
df_2 = df.loc[df[5] == 0]
...
class Net(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(3, 3)  # input transformation
        self.fc2 = nn.Linear(3, 3)  # output transformation
...
def prediction():
    loss_test = []
    x = np.array(df_test.iloc[:, 0:3])
    x = torch.from_numpy(x).type(torch.FloatTensor)
    y = np.array(df_test.iloc[:, 4])
    y = torch.from_numpy(y).type(torch.LongTensor)
    ans = model.predict(x)
